...
def __main__( *args ):
    ...
    scriptDialog.AddControlToGrid( "SequenceConfigLabel", "LabelControl", "Movie Pipeline Config", 5, 0, "The config of level sequence to render.eg. /Game/PathToSequence/SequenceName/EditorMoviePipelineQueue", False )
    scriptDialog.AddControlToGrid( "SequenceConfigBox", "TextControl", "", 5, 1, colSpan=3 )
    ...
    versionBox = scriptDialog.AddComboControlToGrid( "VersionBox", "ComboControl", "5", ("4","5",), 6, 3 )
    ...
    settings = ( "ProjectBox", "MapBox", "LevelSequenceBox", "FramesBox", "SequenceConfigBox","ChunkSizeBox", "VersionBox", "OutputDirBox",
...
def SubmitButtonPressed( *args ):
    ...
    SequenceConfig = scriptDialog.GetValue( "SequenceConfigBox" ).strip()
    if len( SequenceConfig ) == 0:
        errors.append( "Please specify a level sequence Config to render." )
    ...
    for option, value in six.iteritems(jobOptions):
    ...
    writer.WriteLine( "SequenceConfig=%s" % scriptDialog.GetValue( "SequenceConfigBox" ) )
